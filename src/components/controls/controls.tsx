...
const Controls: React.FC<controlProps> = ({ countInBasket, addCount, removeCount, size, sizeControl, stock} ) => {
    ...
    <div className={`${classes.controls} ${classes[size]}` }>
        <button 
            className={`${classes.control} ${sizeControl && classes[sizeControl]}`}  
            onClick={() => removeCount()} 
        ...
        <button 
            ...
            disabled={stock && stock <= countInBasket ? true : false}    
        >
