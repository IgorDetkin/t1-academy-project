...
const Controls: React.FC<controlProps> = ({ countInBasket, addCount, removeCount } ) => {

    const itemWord = `${countInBasket && countInBasket > 1 ? 'items' : 'item'}`

    return (
    <div className={classes.controls}>
        <button className={classes.control}  onClick={() => removeCount()} aria-label='remove from cart'>
        ...
        <span className={classes.count}>{countInBasket} {itemWord}</span>
        <button className={classes.control} onClick={() => addCount()} aria-label='add to cart'>
