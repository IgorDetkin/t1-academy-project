...
const Navigation: React.FC<BasketPriceState> = () => {

  const { totalQuantity} = useSelector((state: RootState) => state.basket);
  ...
  return (
    isAuthenticated && 
    totalQuantity &&
    lastName && //чтобы блок навигации сразу целиком отрисовывался
    ...
    Cart  
      <div className={classes.cart} aria-label='cart icon'>
        ...
        {totalQuantity !== 0 && totalQuantity && <span className={classes.counter}>{totalQuantity}</span>}
      </div>
    </Link>
...
//в этом компоненте нет смысла прописывать ошибку, так как если с запросом auth/me будет что-то не так,
// то до этого компонента мы просто не попадем